...
modelddir = '/pool/asha0/SCIENCE/csalt/storage/radmc/sg_modeld/'
cfiles = [taper1dir+'raw_cube.fits',
          taper2dir+'raw_cube.fits',
          taper3dir+'raw_cube.fits',
          modelddir+'raw_cube.fits']
clbls = ['$\mathsf{a}$', '$\mathsf{b}$', '$\mathsf{c}$', '$\mathsf{d}$']
...
xlims = np.array([2.1666667, -2.1666667])
cm = 'cmr.eclipse'
...
for ic in range(len(cfiles)):
    ...
    for iv in range(nchans):
        ...
        if np.logical_and(ic == 3, iv == 0):
            ax.set_yticks([-2, 0, 2])
            ax.set_xlabel('$\Delta \\alpha \, \cos{\delta}$  ($^{\prime\prime}$)')
